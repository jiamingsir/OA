...
<div class="row" style="padding-top: 15px;">
	<div class="col-md-12">
		...
		<div class="bgc-w box">
			<!--盒子头-->
			<form action="userroleedit" method="post" onsubmit="return check();">
					...
					<div class="box-body">
						...
						<div class="row">
							...
							<div class="col-md-6 form-group" style="colspan:2">
								<div><label class="control-label"> <span>角色</span></div>
								</label>
									...
									<#list roles as role>
										<#if role??>
											<#if role.proRoleValue == 1>
												<div style="width: 200px;float: left;" ><span class="labels"><label><input name="id" type="checkbox" value="${role.id}" checked><i>✓</i></label></span><span  >${role.proRoleName}</span></div>
											<#else >
												<div style="width: 200px;float: left;" ><span class="labels"><label><input name="id" type="checkbox" value="${role.id}" ><i>✓</i></label></span><span  >${role.proRoleName}</span></div>
											</#if>
										<#else >
												</div style="width: 200px;float: left;"><span class="labels"><label><input name="id" type="checkbox" value="${role.id}" checked><i>✓</i></label></span><span  >${role.proRoleName}</span></div>
										</#if>
...
function check() {
	...
	$('.alert-danger').css('display', 'none');
	var isRight = 1;
    var boxes = $("input[name='id']:checked").length;
