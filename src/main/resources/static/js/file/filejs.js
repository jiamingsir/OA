 ...
 * 文件展示 js
 */
$(".loadfiletype2").on("mouseover mouseout",".file-one",function(event){
	if (event.type == "mouseover") {
		// 鼠标悬浮
		$(this).addClass("file-one-houver");
	} else if (event.type == "mouseout") {
...
$(".loadfiletype").on("mousedown",".file-one",function(e){
	if(3 == e.which){
	    ...
	    if($(this).find(".file-img").hasClass("path")){
	    	 ...
	    	 *  给下载a链接添加 href 地址目标
	    	 */
	    	$(".menu .downloadfile2").removeClass("disabled");
	    	var fileid = $(this).find(".filemessage").val();
	    	var filePath = $(this).find(".filePath").val();
	    	var fileName = $(this).find(".fileName").val();
	    	console.log(fileid);
			$(".menu .downloadfile2").attr("href","downfile2?filePath=" + filePath + "&fileName=" + fileName + "");
