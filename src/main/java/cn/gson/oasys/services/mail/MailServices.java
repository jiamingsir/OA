...
public class MailServices {
	...
	public Page<Pagemail> recive(int page,int size,User tu,String val,String title){
		...
		SystemStatusList status=sdao.findByStatusModelAndStatusName("邮件", val);
		SystemTypeList type=tydao.findByTypeModelAndTypeName("邮件", val);
		if(("收件箱").equals(title)){
	...
	public List<Map<String,Object>> mail(Page<Pagemail> mail){
		...
		for (int i = 0; i < maillist.size(); i++) {
			...
			result.put("fileids", maillist.get(i).getMailFileids());
			list.add(result);
	...
	public List<Map<String,Object>> maillist(Page<Inmaillist> mail){
		...
		for (int i = 0; i < maillist.size(); i++) {
			...
			result.put("mailid", maillist.get(i).getMailId());
			result.put("fileid", maillist.get(i).getMailFileid());
			result.put("fileids", maillist.get(i).getMailFiles());
			list.add(result);
	...
	public Attachment upload(MultipartFile file,User mu) throws IllegalStateException, IOException{
		...
		if(!StringUtil.isEmpty(fileName)){
			...
			//String newFileName = UUID.randomUUID().toString().toLowerCase()+"."+suffix;//无序名
    ...
    * @reciver 收件人
    * */
	private static String host = "mail.flairmedia.com.cn";
	/*private static String account = "app@flairmedia.com.cn";
	private static String pass = "xinyun1234";*/
	public Boolean pushmailexchange(String accounts,String password,String reciver,
								 String name,String title,String content,String affix,String filename){
		Model model = null;
		Boolean flag = true;
		String account ="";
		if(accounts.contains(".cn")){
			account = accounts.trim();//
		}else {
			account = accounts.trim()+".cn";//
		}

		  String pass = password;
		  //String host = "mail.flairmedia.com.cn";
		 // 发送邮件对方的邮箱 @flairmedia.com.cn
		String receiveMailAccount ="";
		if (reciver.contains(".cn")){
			receiveMailAccount = reciver.trim();
		}else {
			receiveMailAccount = reciver.trim()+".cn";
		}


		  //String receiveMailAccount = "zhangjiaming@flairmedia.com.cn";

		  if (StringUtil.isEmpty(affix) ){
			  System.out.println("jion in ...");
			  ExchangeClient client = new ExchangeClient.ExchangeClientBuilder()
					  .hostname(host)
					  .exchangeVersion(ExchangeVersion.Exchange2010)
					  .username(account)
					  .password(pass)
					  .recipientTo(receiveMailAccount)
					  .subject(title)
					  .message(content)
					  .build();
			   flag = client.sendExchange();
			  /*if (!flag){
					  model.addAttribute("error", "邮箱信息不正确！");
			  }*/
		  }else {
		  	List<String> filepath = new ArrayList<>();
			  String[] idstr = affix.split(",");
			  for (int i = 0; i < idstr.length; i++) {
				  if (StringUtil.isNotEmpty(idstr[i])){
				  	Long attrid = Long.parseLong(idstr[i]);
					  Attachment attachment = attadao.findOne(attrid);
					  filepath.add(rootpath+attachment.getAttachmentPath());
				  }
			  }
		...
		//.recipientCc(receiveMailAccount)  //抄送
		...
		public void pushmail(String account,String password,String reciver,
		 ...
		 // 1. 创建参数配置, 用于连接邮件服务器的参数配置
	       ...
	       // 开启 SSL 安全连接。
	        ...
	        //                  QQ邮箱的SMTP(SLL)端口为 465或587, 其他邮箱自行去查看 outlook 587)
			//
