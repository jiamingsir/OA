...
public interface PlanDao  extends JpaRepository<Plan, Long>{
	...
	Plan findlatest2(Date start,Date end,long id,long typeid);

	@Query(nativeQuery=true,value="SELECT * from aoa_plan_list p WHERE p.create_time>?1 and p.create_time<?2  and p.plan_user_id=?3 and p.type_id=?4 ORDER BY p.create_time DESC ")
	List<Plan> findlatest(Date start,Date end,long id,long typeid);
	...
	@Query("select p from Plan p where p.planId in( ?1 )  ")
	Page<Plan> findByIds(List<Long> planids,Pageable pa);
