...
public interface RolepowerlistDao extends JpaRepository<Rolepowerlist, Long>{
	...
	Rolepowerlist findbyroleidandmenuid(Long roleid, Long menuid);
		...
		@Query("select  role from Rolepowerlist as role where role.roleId.roleId =?1 and role.check =true ")
		List<Rolepowerlist> findroadids(Long roleid);

	    @Modifying
		@Query("DELETE  from Rolepowerlist as role where role.roleId.roleId =?1")
		int delroadids(Long roleid);
