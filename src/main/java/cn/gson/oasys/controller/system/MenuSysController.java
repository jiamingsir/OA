	...
	public String testMess(HttpServletRequest req, @Valid SystemMenu menu, BindingResult br) {
		...
		// 校验通过，下面写自己的逻辑业务
			...
			if (!StringUtils.isEmpty(session.getAttribute("getId"))) {
				...
				SystemMenu sm =menuService.findByMenuId(parentid);
				if(menu.getMenuName().equals(sm.getMenuName())){

					List<SystemMenu> smlist= menuService.findAllMenuParent();
					Integer list1=smlist.size();
					menu.setMenuUrl("#");
					menu.setParentId(Long.parseLong("0"));
					//menu.setSortId(list1+1);
